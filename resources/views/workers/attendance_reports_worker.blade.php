...
<div class="content-wrapper">
    ...
    <div class="card">
        <br>
        <div class="container">
            <form action="{{url('attendance-report-worker')}}" id="main-form" method="POST">
                @csrf
                <div class="row" id="form-wrapper">
                    ...
                    <div class="form-group col-lg-4 col-md-6">
                        ...
                        <input class="form-control col-lg-7" type="date" id="from_date" name="from" value="{{ isset($data['from'])? date('Y-m-d',strtotime($data['from'])):'' }}" style="display:inline-block" required/>
                    </div>
                    <div class="form-group col-lg-4 col-md-6">
                        <label for="to_date" class="col-lg-4" style="display:inline-block;  text-align:left;">To</label>
                        <input class="form-control col-lg-7" type="date" id="to_date" name="to" value="{{ isset($data['to'])? date('Y-m-d',strtotime($data['to'])):'' }}" style="display:inline-block" required />
                    </div>
                    <div class="form-group col-lg-4 col-md-6">
                        <label class="col-lg-4" for="contractor_id" style="display:inline-block; text-align:left; ">Contractor</label>
                        <select class="form-control col-lg-7" name="contractor_id"  style="display:inline-block">
                            <option value="">All</option>
                            @foreach ($contractors as $contractor)
                                <option
                                {{isset($data['contractor_id'])?(($data['contractor_id']==$contractor->id)?'Selected':''):''}}
                                    value="{{ $contractor->id }}">
                    ...
                    <div class="form-group col-lg-4 col-md-6">
                        <label class="col-lg-4" for="unit" style="display:inline-block; text-align:left; ">Unit</label>
                        <select class="form-control col-lg-7" name="unit"  style="display:inline-block">
                            <option value="">All</option>
                            @foreach ($units as $unit)
                                @if($unit->Product_Villa!='')
                                <option
                                {{  isset($data['unit'])?(($data['unit']==$unit->Booking_ID)?'Selected':''):''  }}
                    ...
                    <div class="form-group col-lg-4 col-md-6">
                        <label class="col-lg-4" for="userid" style="display:inline-block; text-align:left; ">User</label>
                        <select class="form-control col-lg-7" name="userid"  style="display:inline-block">
                            <option value="">All</option>
                            @foreach ($users as $user)
                                <option
                                {{  isset($data['userid'])?(($data['userid']==$user->id)?'Selected':''):''  }}
                                    value="{{ $user->id }}">
                    ...
                    <div class="form-group col-lg-4 col-md-6">
                        <label class="col-lg-4" for="status" style="display:inline-block; text-align:left; ">Status</label>
                        
                                <select class="form-control col-lg-7" name='status' id='status' style="display:inline-block">
                                    <option value=''>All</option>
                                   
                                    <option {{  isset($data['status'])?(($data['status']==2)?'Selected':''):''  }} value=2>
                                        Cancelled
                                    </option>
                                    <option {{  isset($data['status'])?(($data['status']!=2)?'Selected':'' ):'' }} value=3>
                                        Approved
                                    </option>
                                </select>
                    </div>
